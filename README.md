...
This writes `~/.local/share/applications/ringcentral-csv-editor.desktop`
(with `Terminal=false`, so it launches as a windowed app — no terminal) and
...
> **Note:** when launched from a graphical menu the app has no terminal to
> report errors, so make sure the Linux [`libmpv` symlink](#run) is in place
> first — otherwise the window will silently fail to open.
...
> **Linux (one-time):** modern distros ship `libmpv.so.2`, but the Flet client
> links `libmpv.so.1`. If the window never appears, create the compatibility
> symlink (confirmed on Arch + mpv 0.41):
>
> ```bash
...
### Windows installer
An installer is produced automatically by GitHub Actions on each tagged release
(push a `v*` tag, or trigger the workflow manually — see
`.github/workflows/build-windows.yml`): `flet pack` builds the exe, then Inno
Setup (`installer.iss`) packages it. The exe and `…-setup.exe` are attached to
the GitHub Release.
...
| Linux: `libmpv.so.1: cannot open shared object file` | The Flet 0.28 Linux client links the **old** `libmpv.so.1`, but modern distros (Arch, recent Fedora/Ubuntu) ship `libmpv.so.2`. Create a compatibility symlink (confirmed on Arch + mpv 0.41): `sudo ln -s /usr/lib/libmpv.so.2 /usr/lib/libmpv.so.1`. Check the target exists first with `ls /usr/lib/libmpv.so.2*` — the path may differ on other distros. |
